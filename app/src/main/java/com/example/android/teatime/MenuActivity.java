...
public class MenuActivity extends AppCompatActivity implements ImageDownloader.DelayerCallback {
    ...
    /**
     * Returns the IdlingResource variable. It will
    ...
    /**
     * Call downloadImage from ImageDownloader from onStart() to ensure there's enough time
     * for IdlingResource to be initialized. This is because @Before in Espresso Tests
     * is executed after the activity is created in onCreate, so there might not be enough time
     * to register the IdlingResource if the download is done too early.
     */
    @Override
    ...
    // When the thread in ImageDownloader is finished,
    // it returns an ArrayList of Tea objects via the callback.
    @Override
